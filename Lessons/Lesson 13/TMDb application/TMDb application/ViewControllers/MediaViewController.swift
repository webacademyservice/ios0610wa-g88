...
//        mass.remove(at: index)
    ...
    @IBAction func segmentControllDidPressed(_ sender: UISegmentedControl) {
        ...
        switch selectedPage {
        case .movies:
            print("Slected Movies!")
        case .tvShows:
            print("Slected tvShows!")
        }
    }
    ...
    /// Метод запрашивает список актеров и обновляет коллекцию после получения ответа.
    func requestTrendingActors() {
        
        let url = "https://api.themoviedb.org/3/person/popular?api_key=96cfbe0ba15c4721bca8030e8e32becb&language=en-US"
...
extension MediaViewController: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView,
        ...
        if let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "ActorCollectionViewCell", for: indexPath) as? ActorCollectionViewCell {
            cell.configure(actor: actors[indexPath.row])
            return cell
        }
